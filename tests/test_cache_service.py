...
class TestCacheStoreAndRetrieve:
    ...
    @pytest.mark.asyncio
    async def test_store_and_retrieve_exact_match(self, cache_service: SemanticCacheService) -> None:
        ...
        cached = await cache_service.check_cache("Portland cement standards")
...
class TestCacheMiss:
    ...
    @pytest.mark.asyncio
    async def test_cache_miss_on_unrelated_query(self, cache_service: SemanticCacheService) -> None:
        """Retrieve with semantically different query → returns None."""
        resp = _make_pipeline_response(query="Portland cement standards")
        await cache_service.store_cache("Portland cement standards", resp)
        cached = await cache_service.check_cache("xyz completely different unrelated topic")
...
class TestSimilarityThreshold:
    ...
    @pytest.mark.asyncio
    async def test_below_threshold_is_miss(self, cache_service: SemanticCacheService) -> None:
        ...
        cached = await cache_service.check_cache(
...
class TestInvalidateCache:
    ...
    @pytest.mark.asyncio
    async def test_invalidate_clears_all(self, cache_service: SemanticCacheService) -> None:
        ...
        cached = await cache_service.check_cache("Portland cement")
        assert cached is None
...
class TestCacheDbPath:
    ...
    @pytest.mark.asyncio
    async def test_cache_db_path_from_config(self, cache_service: SemanticCacheService) -> None:
